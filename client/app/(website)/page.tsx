    ...
    <div className="space-y-20">
      ...
      >
        ...
        <div className="relative z-10 text-white max-w-4xl">
          <h1 className="text-4xl md:text-6xl lg:text-7xl mb-4 font-bold">Powstruct</h1>
      ...
      {/* About Section */}
      <section className="container mx-auto px-4 md:px-6 py-16 grid grid-cols-1 xl:grid-cols-5 gap-8 md:gap-10 items-center">
        {/* Left Side (Text, takes 2 parts) */}
        <div className="md:col-span-2">
          {/* Qui Sommes-Nous */}
          <h2 className="text-2xl md:text-3xl font-bold mb-4">Qui Sommes-Nous ?</h2>
          <p className="text-gray-700 leading-relaxed mb-8 md:mb-10">
        ...
        <div className="relative md:col-span-3 rounded-3xl overflow-hidden w-full max-w-[500px] md:max-w-[700px] aspect-square mx-auto">
          <Image
            src="/about_img.jpg"
          ...
          <div className="absolute inset-0 m-4 md:m-8 lg:m-12 rounded-3xl border-2 border-[#FBD96D] pointer-events-none"></div>
        </div>
      </section>

      {/* Values Section */}
      <section className="container mx-auto py-16 px-4 md:px-6 lg:px-12">
        <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-8 md:mb-12 text-center">Nos Valeurs Fondamentales</h2>
        
        {/* Mobile Layout - Content Over Image */}
        <div className="md:hidden relative rounded-3xl min-h-[800px] mb-8">
          {/* Background Image */}
          <Image
            src="/about_img.jpg"
            alt="Cheminée Powstruct"
          ...
          {/* Content Overlay - Now with proper height */}
          <div className="absolute inset-0 flex flex-col justify-center items-center p-6">
            <div className="grid grid-cols-1 gap-4 h-fit max-h-full overflow-y-auto py-4">
              {valeurs.map((item, i) => (
                <div key={i} className="bg-black/60 backdrop-blur-sm p-4 rounded-xl">
                  <div className="flex items-center gap-2 mb-2">
                    {item.icon}
                    <h3 className="font-semibold text-white text-sm md:text-base">{item.title}</h3>
                  </div>
                  <p className="text-white/90 text-xs md:text-sm">{item.desc}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="hidden md:grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8 text-left">
          {valeurs.map((item, i) => (
        ...
        {/* Image with border overlay (Desktop only) */}
        <div className="hidden md:block relative mt-12 md:mt-16 rounded-3xl overflow-hidden h-[250px] md:h-[350px] lg:h-[400px] mx-4">
          <Image
            src="/about_img.jpg"
            alt="Cheminée Powstruct"
            fill
            className="object-cover object-center rounded-3xl"
          />
          <div className="absolute inset-0 rounded-3xl m-4 md:m-8 lg:m-12 border-2 border-[#FBD96D] pointer-events-none"></div>
        </div>
      </section>

      {/* Products Section */}
      <section className="container mx-auto py-16 px-4 md:px-6 lg:px-12">
        <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-center mb-8 md:mb-12">Nos Produits</h2>

        <div className="space-y-12 md:space-y-16">
          {produits.map((item, i) => (
            <div
              key={i}
              className={`flex flex-col ${i % 2 !== 0 ? 'md:flex-row-reverse' : 'md:flex-row'} items-center gap-6 md:gap-8`}
            >
              {/* Text (1/3 width) */}
              <div className={`md:w-1/3 ${i % 2 !== 0 ? 'md:text-right' : 'md:text-left'} text-center`}>
                <h3 className="text-2xl md:text-3xl lg:text-4xl font-semibold mb-4">{item.title}</h3>
                <p className="text-gray-600 text-sm md:text-base">{item.desc}</p>
              </div>

              {/* Image (2/3 width) */}
              <div className="relative md:w-2/3 w-full h-[200px] md:h-[300px] lg:h-[350px] rounded-3xl overflow-hidden">
                {/* Vector corner shape */}
                <div className="absolute top-0 left-0 w-12 h-12 md:w-16 md:h-16 bg-[#FBD96D] rounded-br-3xl z-20"></div>
                ...
                {/* Yellow border overlay */}
                <div className="absolute inset-0 rounded-3xl m-4 md:m-6 lg:m-8 border-2 border-[#FBD96D] pointer-events-none"></div>

                {/* Overlay text */}
                <div className="absolute bottom-[40px] md:bottom-[60px] left-1/2 transform -translate-x-1/2 text-[#FBD96D] font-semibold text-lg md:text-xl text-center">
                  <p>Voir Notre Collection</p>
                  <div className="w-28 md:w-36 h-[4px] md:h-[6px] rounded-2xl bg-[#FBD96D] mt-1 mx-auto"></div>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="max-w-4xl text-lg md:text-xl font-semibold mx-auto mt-12 p-4 text-center">
          <p className="mb-4">
